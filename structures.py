...
class Position(ABC):
    ...
    def compute_action(self, logit: torch.Tensor) -> torch.Tensor:
        ...
        if self.mode == "mse":
            ...
            return self._disc_to_cont(logit.argmax(2))
        else:
    ...
    def compute_loss(self, pred: Output, sample: Sample) -> Dict[str, torch.Tensor]:
        ...
        if self.mode == "mse":
            ...
            logit = pred["position"]
            position = action[:, :3]
            return {"position": F.mse_loss(logit, position) * 3}

        elif self.mode == "ce":
            raise NotImplementedError()
            logit = pred["attention"]
            # position = sample.
            return self._compute_ce_loss(logit, position)
    ...
    def _compute_ce_loss(
        ...
        true_disc = self._cont_to_disc(true_cont).float()
    ...
    def _disc_to_cont(self, position: torch.Tensor):
        self.workspace = self.workspace.to(position.device)
        self.size = self.size.to(position.device)

        low_bound = self.workspace[0]
        position = position / self.resolution * self.size + low_bound

        nan = position.isnan().any(-1)
        if nan.any():
            raise RuntimeError("The position should never be nan")

        return position

    def _cont_to_disc(self, cont: torch.Tensor) -> torch.Tensor:
        self.workspace = self.workspace.to(cont.device)
        self.size = self.size.to(cont.device)

        low_bound = self.workspace[0]
        discrete = (cont - low_bound) / self.size * self.resolution
...
class Rotation(ABC):
    ...
    def _compute_mse_loss(
        ...
        loss = F.mse_loss(logit, true, reduction="none").to(dtype)
        return {f"rotation": loss}
