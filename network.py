...
class CrossTransformer(nn.Module):
    ...
    def _add_taskvar(
        ...
        task_emb = einops.repeat(task_emb, "b d -> (b t) k d", t=T, k=1)

        var_emb = self.var_embedding(taskvar[:, 1].long())
        var_emb = self.var_norm(var_emb)
        var_emb = einops.repeat(var_emb, "b d -> (b t) k d", t=T, k=1)
    ...
    def forward(
        ...
        if instruction is not None:
            if self._avg_instr:
                instruction = instruction.mean(1, keepdim=True)
...
class PlainUNet(nn.Module):
    ...
    def compute_action(self, pred: Output) -> torch.Tensor:
        ...
        position = self._pos.compute_action(pred["position"])

        return torch.cat(
            [position, rotation, pred["gripper"]],
            dim=1,
        )
    ...
    def head(
        ...
        if self._z_mode == "embed":
            ...
            offset = xtr[:, -3:]
            offset = einops.reduce(offset, "(b n) ch h w -> b ch", "mean", n=N)
            position += offset
        else:
...
class TransformerUNet(PlainUNet):
    ...
    def _gaussian_noise(self, x: torch.Tensor, sigma: float = 0.05) -> torch.Tensor:
        if self.training and sigma != 0:
            ...
            sampled_noise = torch.empty_like(x).normal_() * scale
            x = x + sampled_noise
        return x
    ...
    def forward(
        ...
        if self._pcd_noise:
            pc_obs = self._gaussian_noise(pc_obs)
        ...
        if self._pcd_token != "none":
            ...
            if self._pcd_token == "max":
                pcd = F.max_pool2d(pcd, 16)
            elif self._pcd_token == "mean":
                pcd = F.avg_pool2d(pcd, 16)
            elif self._pcd_token == "median":
                pcd = MedianPool2d(16, stride=16)(pcd)
            else:
                raise RuntimeError(f"Unexpected pcd token {self._pcd_token}")
