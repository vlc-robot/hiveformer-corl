...
from models_without_hydra.model import Model
...
class ResNetBase(Model):
    ...
    def __init__(self,
                 in_channels, out_channels, dilations,
                 bn_momentum, conv1_kernel_size, D=3, **kwargs):
        assert self.BLOCK is not None
        assert self.OUT_PIXEL_DIST > 0

        super().__init__(in_channels, out_channels, D, **kwargs)

        self.network_initialization(in_channels, out_channels, dilations,
                                    bn_momentum, conv1_kernel_size, D)
